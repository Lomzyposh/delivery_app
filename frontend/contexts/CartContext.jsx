...
const API = 'http://192.168.121.224:5000'
...
export function CartProvider({ children }) {
    ...
    useEffect(() => {
        ...
        (async () => {
            try {
                setLoading(true);
                const data = await safeJsonFetch(`${API}/cart?userId=${encodeURIComponent(userId)}`);
    ...
    const addToCart = async ({ foodId, quantity = 1, addons = [], notes }) => {
        const data = await safeJsonFetch(`${API}/cart/items`, {
            method: "POST",
    ...
    const setQuantity = async (itemId, quantity) => {
        const data = await safeJsonFetch(`${API}/cart/items/${itemId}/quantity`, {
            method: "PATCH",
            headers: { "Content-Type": "application/json" },
    ...
    const replaceAddons = async (itemId, addons) => {
        const data = await safeJsonFetch(`${API}/cart/items/${itemId}/addons`, {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
    ...
    const removeItem = async (itemId) => {
        const data = await safeJsonFetch(`${API}/cart/items/${itemId}`, {
            method: "DELETE",
            headers: { "Content-Type": "application/json" },
    ...
    const clearCart = async () => {
        const data = await safeJsonFetch(`${API}/cart/clear`, {
            method: "DELETE",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ userId }),
        });
